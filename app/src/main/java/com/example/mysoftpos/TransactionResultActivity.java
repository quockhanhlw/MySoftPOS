...
public class TransactionResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (type == null) type = ResultType.SYSTEM_ERROR;

        switch (type) {
            case SUCCESS:
                setupUI(ivIcon, tvTitle, btnClose, R.drawable.ic_check_circle, "#4CAF50", "Giao dịch Thành công", "Hoàn tất");
                ...
                if (txnType == com.example.mysoftpos.iso8583.TxnType.BALANCE_INQUIRY && isoResponse != null) {
                    try {
                        ...
                        byte[] rawResp = com.example.mysoftpos.utils.StandardIsoPacker.hexToBytes(isoResponse);
                        
                        // 2. Unpack DE 54
                        String f54 = com.example.mysoftpos.utils.StandardIsoPacker.unpackField(rawResp, 54);
                        ...
                        if (f54 != null) {
                            ...
                            tvReason.setText(messageExtras != null ? messageExtras : "Không tìm thấy thông tin số dư (DE 54)");
                        }
                    } catch (Exception e) {
            ...
            case LIMIT_EXCEEDED:
                setupUI(ivIcon, tvTitle, btnClose, R.drawable.ic_warning, "#FF9800", "Vượt quá hạn mức", "Nhập lại số tiền"); // Orange
            ...
            case INVALID_CARD:
                setupUI(ivIcon, tvTitle, btnClose, R.drawable.ic_credit_card_off, "#F44336", "Thẻ không hợp lệ", "Thử lại"); // Red
                tvReason.setText("Số thẻ không hợp lệ hoặc không được hỗ trợ.");
                break;

            case SYSTEM_ERROR:
                setupUI(ivIcon, tvTitle, btnClose, R.drawable.ic_cloud_off, "#607D8B", "Lỗi kết nối", "Về màn hình chính"); // Blue Grey
                tvReason.setText(messageExtras != null ? messageExtras : "Lỗi hệ thống hoặc mất kết nối.");
                break;

            case TRANSACTION_FAILED:
                setupUI(ivIcon, tvTitle, btnClose, R.drawable.ic_error, "#F44336", "Giao dịch Thất bại", "Về màn hình chính"); // Red
    ...
    private void setupUI(ImageView iv, TextView tv, MaterialButton btn, int resId, String colorHex, String title, String btnText) {
