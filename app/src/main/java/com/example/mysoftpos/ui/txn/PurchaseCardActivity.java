package com.example.mysoftpos.ui.txn;
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                runOnUiThread(() -> {
                    ...
                    showResult("00".equals(rc), msg, StandardIsoPacker.bytesToHex(resp),
    ...
    private void handleAutoReversal(TransactionContext ctx, CardInputData card, TransactionEntity entity) {
        try {
            ...
            try {
                ...
                runOnUiThread(() -> {
                    showLoading(false);
                    // Specific User Request: "hết timeout sau mà vẫn không nhận được 0430 thì hiện
                    // giao dịch lỗi time out"
            ...
            com.example.mysoftpos.utils.FileLogger.logString(this, "ERROR",
                    "Reversal Critical Error: " + e.getMessage());
