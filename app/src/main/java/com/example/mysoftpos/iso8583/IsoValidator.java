...
public final class IsoValidator {
    ...
    public static void validateAgainstNapasSpec(String mti, Map<Integer, String> fields, boolean validateMti, String contextName) {
        ...
        for (Map.Entry<Integer, String> e : fields.entrySet()) {
            ...
            // Skip validation for mock fields or variable text fields to avoid false positives
    ...
    private static void validateValueBySpec(int de, String value, NapasFieldSpecConfig.FieldSpec spec, String contextName) {
        if (value == null) {
        ...
        if (len < spec.minLen || len > spec.maxLen) {
            throw new IllegalStateException(
                    contextName + " invalid DE" + de + " length=" + len + " expected " + spec.minLen + ".." + spec.maxLen
                            + " (" + spec.errorCode + ")"
            );
        }
    ...
    private static void validatePosEntryMode22(IsoMessage msg, String name) {
        ...
        if (isBlank(f22)) return;
        if (!f22.matches("\\d{3}")) {
            throw new IllegalStateException(name + " F22 must be 3n. Got: " + f22);
        }

        if (!("012".equals(f22) || "051".equals(f22) || f22.startsWith("05") || f22.startsWith("07") || f22.startsWith("01"))) {
        ...
        if (!isBlank(entryMode)) {
            if (entryMode.startsWith("05") || entryMode.startsWith("07")) {
                if (!msg.hasField(IsoField.ICC_DATA_55) || isBlank(msg.getField(IsoField.ICC_DATA_55))) {
                    throw new IllegalStateException(context + " requires field 55 for ICC entry mode (F22=" + entryMode + ")");
                }
            }
        }
    }
    ...
    private static void validateField60IfPresent(IsoMessage msg, String name) {
        ...
        if (f60.matches("\\d{27}")) {
            ...
            if (!reserved1.equals("00000000")) throw new IllegalStateException(name + " F60 UPI reserved(5-12) must be 00000000");
