...
public final class NapasFieldSpecConfig {
    ...
    private static Map<Integer, FieldSpec> build() {
        ...
        m.put(1, new FieldSpec(1, 16, 16, "[0-9A-Fa-f]{16}", "NPS-001", "Secondary Bitmap Error: Must be 16 Hex characters"));
        ...
        m.put(7, new FieldSpec(7, 10, 10, "\\d{10}", "NPS-007", "Transmission Date/Time Error: Must be 10 numeric digits (MMDDhhmmss)"));
        ...
        m.put(14, new FieldSpec(14, 4, 4, "\\d{4}", "NPS-014", "Expiration Date Error: Must be 4 numeric digits (YYMM)"));

        // DE 15: Settlement Date - n4 (MMDD)
        m.put(15, new FieldSpec(15, 4, 4, "\\d{4}", "NPS-015", "Settlement Date Error: Must be 4 numeric digits (MMDD)"));
        ...
        m.put(32, new FieldSpec(32, 1, 11, "\\d{1,11}", "NPS-032", "Acquirer ID Error: Must be numeric, max length 11 (LLVAR)"));

        // DE 35: Track 2 Data - z..37; LLVAR
        m.put(35, new FieldSpec(35, 1, 37, "[0-9=D]{1,37}", "NPS-035", "Track 2 Error: Invalid format or length > 37 (LLVAR)"));

        // DE 36: Track 3 Data - z..104; LLVAR
        m.put(36, new FieldSpec(36, 1, 104, "[0-9=D]{1,104}", "NPS-036", "Track 3 Error: Invalid format or length > 104 (LLVAR)"));

        // DE 37: Retrieval Reference Number (RRN) - an12
        m.put(37, new FieldSpec(37, 12, 12, "[a-zA-Z0-9]{12}", "NPS-037", "RRN Error: Must be 12 alphanumeric characters"));
        ...
        m.put(39, new FieldSpec(39, 2, 2, "[a-zA-Z0-9]{2}", "NPS-039", "Response Code Error: Must be 2 alphanumeric characters"));
        ...
        m.put(52, new FieldSpec(52, 16, 16, "[0-9A-Fa-f]{16}", "NPS-052", "PIN Block Error: Must be 16 Hex characters"));

        // DE 54: Additional Amounts - ans..120; LLLVAR
        m.put(54, new FieldSpec(54, 1, 120, "[ -~]{1,120}", "NPS-054", "Additional Amounts Error: Max length 120 (LLLVAR)"));

        // DE 55: Chip Data - b..255 (Hex string length 510); LLLVAR
        m.put(55, new FieldSpec(55, 1, 510, "[0-9A-Fa-f]{1,510}", "NPS-055", "ICC Data Error: Must be Hex string, max length 510 (LLLVAR)"));
        ...
        m.put(100, new FieldSpec(100, 1, 11, "\\d{1,11}", "NPS-100", "Receiving Inst. Code Error: Max length 11 (LLVAR)"));

        // DE 102: From Account Identification - an...28, LLVAR
        m.put(102, new FieldSpec(102, 1, 28, "[a-zA-Z0-9]{1,28}", "NPS-102", "From Account Error: Alphanumeric, max length 28 (LLVAR)"));

        // DE 103: To Account Identification - an...28, LLVAR
        m.put(103, new FieldSpec(103, 1, 28, "[a-zA-Z0-9]{1,28}", "NPS-103", "To Account Error: Alphanumeric, max length 28 (LLVAR)"));

        // DE 104: Content Transfer - ans...210, LLLVAR
        m.put(104, new FieldSpec(104, 1, 210, "[ -~]{1,210}", "NPS-104", "Content Transfer Error: Max length 210 (LLLVAR)"));

        // DE 105: New PIN Block - ans...999, LLLVAR
        m.put(105, new FieldSpec(105, 1, 999, "[ -~]{1,999}", "NPS-105", "New PIN Block Error: Max length 999 (LLLVAR)"));

        // DE 120: Beneficial Card holder Info - ans...70, LLLVAR
        m.put(120, new FieldSpec(120, 1, 70, "[ -~]{1,70}", "NPS-120", "Beneficial Info Error: Max length 70 (LLLVAR)"));

        // DE 128: Message Authentication Code - an16
        m.put(128, new FieldSpec(128, 16, 16, "[a-zA-Z0-9]{16}", "NPS-128", "MAC Error: Must be 16 alphanumeric characters"));
